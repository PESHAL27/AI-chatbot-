...
export const UserProfileModal: React.FC<UserProfileModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      <div className="w-full max-w-md glass-panel rounded-3xl p-6 border border-purple-500/40 shadow-[0_0_60px_rgba(168,85,247,0.25)] relative pml-neon-card">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-5">
          <div className="flex items-center gap-2">
            <div className="p-1.5 rounded-lg bg-black/60 border border-purple-500/30">
              <img src="/assets/plm_symbol.png" alt="PML" className="w-5 h-5 rounded-full object-cover" />
            </div>
            <h2 className="font-display font-bold text-xl text-white">PML Neural Identity</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="text-center mb-6">
          <div className="w-20 h-20 mx-auto rounded-full bg-gradient-to-tr from-violet-600 via-indigo-600 to-purple-600 p-1 shadow-[0_0_25px_rgba(147,51,234,0.5)] mb-3">
          ...
          <p className="text-xs text-purple-300 font-mono mb-2">{displayEmail}</p>
        ...
        {user?.id && (
          <div className="p-3 mb-4 rounded-xl bg-black/60 border border-purple-500/20 flex flex-col gap-1 text-left font-mono">
            <span className="text-[10px] text-slate-400 uppercase tracking-widest">User Neural ID</span>
            <span className="text-xs text-purple-300 truncate select-all">{user.id}</span>
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          ...
          <div className="p-3 rounded-2xl bg-black/50 border border-purple-500/20 text-center">
            <FileCheck className="w-4 h-4 mx-auto text-indigo-400 mb-1" />
        ...
        {onOpenMemory && (
          <button
            ...
            className="w-full mb-4 p-3 rounded-2xl bg-gradient-to-r from-purple-950/60 to-black/60 border border-purple-500/30 hover:border-purple-500/60 flex items-center justify-between text-left transition-colors cursor-pointer group"
          >
            ...
            <ExternalLink className="w-4 h-4 text-slate-500 group-hover:text-purple-300" />
          </button>
        )}

        {/* Joined Metadata */}
        <div className="p-3 mb-6 rounded-xl bg-black/50 border border-purple-500/20 flex items-center justify-between text-xs text-slate-400 font-mono">
          <span>Active Since</span>
          <span className="text-purple-300 font-bold">{joinedYear}</span>
        </div>
        ...
        <div className="space-y-3">
          <button
            ...
            className="w-full py-3 rounded-xl bg-purple-950/80 hover:bg-purple-900/90 border border-purple-500/60 text-purple-200 font-display font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(168,85,247,0.25)] hover:shadow-[0_0_30px_rgba(168,85,247,0.45)] transition-all cursor-pointer"
          >
